...
describe("serializeConfig / parseConfig", () => {
  ...
  it("returns null for corrupt JSON, non-objects, and wrong versions", () => {
    ...
    // A version past the current schema: the host treats null as corrupt and
    // quarantines, which is what keeps a downgrade from eating a newer file.
    expect(parseConfig(`{"version":${CONFIG_VERSION + 1}}`)).toBeNull();
    expect(parseConfig("{}")).toBeNull();
  });
      ...
      templates: [
        {
          ...
          cols: 2,
          rows: 1,
          slots: [{ col: 0, row: 0, w: 2, h: 1 }],
        },
        {
          id: "tpl:user:2",
          name: "slot out of bounds",
          cols: 2,
          rows: 1,
          slots: [{ col: 1, row: 0, w: 2, h: 1 }],
        },
        {
          id: "tpl:user:3",
          name: "bad slot type",
...
describe("exportConfig / constructor round-trip", () => {
  ...
  it("starts the grid empty (no throw) on a missing layouts entry", () => {
    ...
    expect(() =>
      brain.enableGrid(cfg.grids[0]!, [makeWindow("1")]),
    ).not.toThrow();
  ...
  it("exportConfig keeps builtin templates exactly once through repeated round-trips", () => {
    ...
    expect(cfg2.templates.filter((t) => t.builtin).map((t) => t.id)).toEqual(
      builtinIds,
    );
