...
| Windows that refuse focus | `WS_EX_NOACTIVATE` | Tooltips, dropdown popups, IME candidate lists, notification toasts. WebView2 renders tooltips as real top-level windows — `Chrome_WidgetWin_1`, no title, no owner, `WS_POPUP`, and *not* `WS_EX_TOOLWINDOW` — so hovering a tile in the settings map put a tooltip in the grid as "Window 1776002" and shoved the real windows aside. Activation is the right test: it asks whether the user can work in the window rather than how it is decorated, so the borderless windows the caption gate was dropped for stay managed |
