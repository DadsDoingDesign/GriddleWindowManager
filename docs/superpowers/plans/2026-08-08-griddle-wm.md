...
Contract note (minimal-move reflow solver — `packages/brain/src/reflow.ts`): **no contract change.** `solveMinimalMoves(tiles, target, dims, opts?)` is a pure function — plain data in, plain data out, no `@griddle/core`, no Tauri, no DOM, no Node — so `ipc.rs`, the C1 types and the capability files are all untouched, and the marketing site can bundle the identical file (one algorithm, two consumers, no drift). It answers the question the `addTileWithDisplacement` path does not: *place the dropped tile exactly where the user aimed, then reorganise the others, moving as few of them as possible.* Objective is lexicographic — fewest tiles moved, then smallest total Manhattan distance — and the search is an iterative deepening over the move budget, with displaced tiles discovered as blockers rather than guessed as subsets, so the first budget that succeeds is optimal in count. It is deterministic (tiles are sorted by id first, so the answer depends on the board and not on the caller's array order), bounded by `REFLOW_DEFAULT_MAX_NODES` candidate positions, and total: nonsense input (bad dims, out-of-bounds or overlapping tiles, duplicate/empty ids) returns `null`; a board it cannot solve inside the cap returns `{ok: false}` so the caller falls back to the existing push behaviour. The bound covers the whole solve, not just the tree walk: a tile's list of legal homes is built only when the search first lifts that tile (a drop lifts a handful, not all `n`), and enumerating it charges its own length against the same budget — so past O(n log n + cols·rows) of setup, `maxNodes` is the only thing that decides how long the call runs. The full default cap costs single-digit milliseconds even on the largest input the function accepts (64x64, 4095 tiles). `moves` lists displaced tiles only — a `target.id` that names an existing tile is the dragged tile itself, re-placed rather than displaced, and never reported. Wiring it into a C3 grid mode is a separate task and will need its own contract paragraph.

Contract extension (**placement mode** — wiring the minimal-move solver into a grid mode, the task the paragraph above deferred). **No new event or command names**, but the **C1 contract changes in both mirrors** (`packages/brain/src/types.ts` + `src-tauri/src/ipc.rs`): `GridSettings.mode` becomes the three-way `PlacementMode = 'reflow' | 'push' | 'stack'`, replacing `'collision' | 'overlay'`.
- **Rename, not re-behavior:** `collision` was push and `overlay` was stack, so the two shipped modes keep their exact semantics under new names. Both loaders still accept the old spellings and rewrite them — TS `normalizePlacementMode` (persist.ts, used by `sanitizeConfig` *and* by the brain's constructor intake, since the shipped read path is Rust serde straight into the constructor), Rust `#[serde(alias)]` on `GridMode`. `AppConfig` bumps to **`version: 4`**: `sanitizeConfig` accepts versions 1–4 and emits 4, `config.rs` accepts `MIN_CONFIG_VERSION..=CONFIG_VERSION` and re-stamps; unknown *future* versions still quarantine as `.bak` + fresh start. `setMode(gridId, mode)` accepts the legacy names too and ignores anything else.
- **Reflow mode** (C3 semantics): on a drop — native drag, resize, cross-grid transfer, or a settings-editor move, all of which funnel through `commitSameGrid`/`commitTransfer` — the dragged tile takes the aimed slot *exactly* and `solveMinimalMoves` (see the note above) reorganises the other **in-flow** tiles around it. The solved arrangement is installed in bulk via Griddle's `snapshotTiles`/`restoreTiles`, so no displacement rule second-guesses it and no tile is removed and re-added; the dragged window and every displaced neighbor therefore land in the **one** `ApplyLayout` the commit's `flush()` emits. The solver declines — nonsense input, an unsolvable board, its node cap, or (on a spanning grid, whose dead cells it knows nothing about) a solution that strands a tile in the seam — by returning `null`/`ok: false`, and the commit then falls back to the push path unchanged. So reflow is never worse than push.
- **Preview parity:** `dragMoved` previews a reflow grid with the solver's own answer (`previewGhosts` → `solveReflow` → ghosts), and falls back to the push simulation exactly when the commit will. What the overlay's ghosts show is what the release delivers.
- **Unchanged in reflow mode:** auto-placement (`windowAppeared`, enable-grid sweeps, app rules, templates, view claims) is still first-fit + `addTileWithDisplacement` — the solver is a *drop* rule, not a placement rule. Absolute tiles (stack leftovers, non-resizable windows) stay out of flow: they neither block nor get moved by a solve. `push ⇄ reflow` moves no tile at all (both keep the same in-flow tiles); only crossing the stack boundary converts tiles.
- **Default:** `DEFAULT_PLACEMENT_MODE = 'reflow'` (exported from `brain.ts`) applies to **newly created grids only** — the brain host's enable paths still prefer a grid's remembered mode, and the v4 migration preserves every existing grid's mode, so upgrading changes nothing about an existing desktop.
- **Settings UI:** the per-grid mode segmented control becomes a `<select>` dropdown listing the three modes with a one-line description each (monitor cards and span cards alike), persisting on change and re-applying live through the existing `settings-set-mode {gridId, mode}` event — whose payload type is now `PlacementMode`.
